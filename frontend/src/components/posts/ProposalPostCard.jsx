...
const ProposalPostCard = ({ post }) => {
    ...
    return (
        <div className="group relative mb-3 transition-all duration-300">
            {/* Background & Decoration Wrapper (Handles clipped elements like the gradient) */}
            <div className="absolute inset-0 bg-white dark:bg-slate-900 border border-gray-100 dark:border-slate-800 rounded-2xl shadow-sm group-hover:shadow-xl group-hover:shadow-blue-500/5 transition-all duration-300 overflow-hidden pointer-events-none" />
            ...
            {/* Content Container (NO overflow-hidden, so actions menu can float over) */}
            <div className="relative p-5 z-10">
                ...
                {attachments && attachments.length > 0 && (
                    <div className="mb-3 bg-gray-50 dark:bg-slate-800/40 border border-gray-100 dark:border-slate-700 rounded-xl p-3 relative z-20">
                ...
                {/* Footer */}
                <div className="mt-3 pt-3 border-t border-gray-50 dark:border-slate-800 flex items-center justify-between relative z-20">
                    {/* Interests */}
                    ...
                    {currentUser?.uid !== user?.uid && (
                        <button
                            ...
                            disabled={status === "group_formed"}
                            className={`cursor-pointer text-sm font-semibold px-5 py-2 rounded-full active:scale-95 transition-all shadow-lg shadow-black/10 dark:shadow-white/5 ${status === "group_formed"
                                ? "bg-gray-200 dark:bg-gray-700 text-gray-500 dark:text-gray-400 cursor-not-allowed uppercase"
                                : "bg-black dark:bg-white text-white dark:text-black hover:opacity-80"
                                }`}
                        >
                            {status === "group_formed" ? "Full" : "Request"}
                        </button>
                    )}
                </div>
            </div>
